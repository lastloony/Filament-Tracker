...
Инструкция для этапа 10 из `TZ.md`. Написана заранее, до реального сервера — команды не обкатаны в бою, при первом деплое сверяться и поправлять по месту.
...
`caddy/Caddyfile` с реальным доменом не коммитить — он в `.gitignore` намеренно (см. `docs/TZ.md`).
