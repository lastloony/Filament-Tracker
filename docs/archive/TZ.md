> **Архив.** Исходное ТЗ, зафиксировано на момент старта разработки. Базовая реализация (backend, frontend, Docker/Caddy-деплой) завершена и может отличаться от текущего кода в деталях. Актуальное состояние — [`README.md`](../../README.md), планы на будущее — [`docs/ROADMAP.md`](../ROADMAP.md). Этот файл не обновляется.
